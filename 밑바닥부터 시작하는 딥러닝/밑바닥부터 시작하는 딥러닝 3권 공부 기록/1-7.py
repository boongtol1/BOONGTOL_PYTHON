...
a = A(x)    # --> A의 input 에 x 를 대입해라!--> A의 초기화변수 self.input에 x가 대입된다!
b = B(a)    # --> B의 input 에 a 를 대입해라!--> B의 초기화변수 self.input에 a가 대입된다!
y = C(b)    # --> C의 input 에 b 를 대입해라!--> C의 초기화변수 self.input에 b가 대입된다!
# backward
y.grad = np.array(1.0)
y.backward()
print(x.grad)